...
以下は全 5 節のうち 4 節の抜粋です。省略したのは「着目すべき ADM フェーズ」「推奨技法」「作るべき成果物」の 3 節で、そこには TOGAF のフェーズ ID・技法 ID・成果物 ID の一覧が入ります。

*Excerpt: the phase / technique / deliverable listings are omitted below.*
...
*記述が無いため判断していない観点: 体制 / 既存資料 / 決定権 / 現場の空気 / 規制。ここを書き足すと助言が変わります。*
...
今日やることを 1 つに絞るなら: 並行できる作業(現状調査、データの棚卸し、移行方式の比較検証)に先に人を投入する。ただし決定そのものは外に出さない。外注できるのは調査までで、判断は自分たちが持つ。
...
同じ 4 節を抜粋しています。 / The same four sections.
...
*記述が無いため判断していない観点: 期限 / 既存資料 / 決定権 / 現場の空気 / 規制。ここを書き足すと助言が変わります。*
...
同じトピックで、**推奨技法の順位が入れ替わり、今日やることと最初に聞く質問が別物になっています。** 「予算が無いなら準備度を測れ」「予算があるなら今のうちに減らす決定を通せ」は、どちらも TOGAF の本には書かれていない実務判断です。
...
*記述が無いため判断していない観点: 予算 / 期限 / 経営の関与 / 体制 / 既存資料 / 決定権 / 現場の空気 / 規制。ここを書き足すと助言が変わります。*
...
… 他 16 件(kind を個別に指定すると全件表示します)

_出典: `security-report.md` (行番号は元ファイル基準)_

##### ステークホルダー (6)

| 行 | 関係者候補 | 役職 | 所属 | 影響度推定 | 原文(出典) |
| --- | --- | --- | --- | --- | --- |
| 39 | IT 基盤部 | 責任部門 | — | medium | 特権 ID の棚卸しが 18 か月間未実施 \| 責任部門: IT 基盤部 |
| 51 | 販売システム部 | 責任部門 | — | medium | 受注ポータルの認証情報が平文で保存 \| 責任部門: 販売システム部 |
| 53 | 山田 | 是正責任者 | 販売システム部 | high | 受注ポータルの認証情報が平文で保存 \| 是正責任者: 山田(販売システム部) |
| 62 | 調達部 | 責任部門 | — | medium | 委託先における再委託の未把握 \| 責任部門: 調達部、法務部 |
| 62 | 法務部 | 責任部門 | — | medium | 委託先における再委託の未把握 \| 責任部門: 調達部、法務部 |
| 75 | 伊藤 | 是正責任者 | IT 基盤部 | high | インシデント対応の責任分界が未定義 \| 是正責任者: 伊藤(IT 基盤部) |
...
*(…risks 24 件・stakeholders 6 件・actions 8 件・notes 10 件が続く。抜粋のため省略)*
...
**プレビューのみです。まだ何も保存していません。** 内容を確認したうえで登録するなら apply=true で再実行してください(候補 51 件、うち登録対象 49 件)。
...
- `export_archimate_open_exchange` の XML は**内容から ID を決めるため一致します**(上の 3 点の例外)
